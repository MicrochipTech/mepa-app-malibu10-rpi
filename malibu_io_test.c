...
mepa_rc appl_mepa_phy_init(mepa_port_no_t port_no)
{
    ...
    // NOTE: Code below is commented-out since mepa_conf_get is not implemented
    // for Malibu PHYs. See mepa_malibu_driver_init() in mepa/vtss/src/vtss.c
    // and you'll notice that only mepa_driver_conf_set() is populated, but not
    // mepa_driver_conf_get().
    // MJ Addition 2025-09-02: As of 2025.06, mepa_driver_conf_get() is now populated
    // and points to phy_10g_conf_get()
    ...
    // MJ Addition 2025-09-02: Comment out code related to calling vtss api directly
    // memset(&oper_mode, 0, sizeof(vtss_phy_10g_mode_t));
    // oper_mode.oper_mode = VTSS_PHY_LAN_MODE;
    // oper_mode.interface = VTSS_PHY_SFI_XFI;     // From vtss_appl_10g_phy_malibu.c
    // oper_mode.channel_id = VTSS_CHANNEL_AUTO;   // From vtss_appl_10g_phy_malibu.c
    ...
    // oper_mode.polarity.line_tx = (port_no < 2)? false : true;
    ...
    // Update config based on code above.
    ...
    // MJ Addition 2025-09-02: Commenting out code below to use MEPA above.
    // rc = (mepa_rc)vtss_phy_10g_mode_set(NULL, port_no, &oper_mode);
    // if (rc != MEPA_RC_OK)
    // {
